...
var CLOUDINARY_URL = 'https://api.cloudinary.com/v1_1/dedsqkslr/upload';
var CLOUDINARY_UPLOAD_PRESET = 'bghh0qb0';
...
function nightTime(){
    ...
    if(checkBox.checked == true){
        nightMode = true;
        chat_page.style.backgroundColor = "	#141d26";
        chatWrapper.style.backgroundColor = "#243447";
        chatWrapper.style.border = '2px solid #243447'
    }else{
        chat_page.style.backgroundColor = "#ffffff"
    }
}
...
socket.on('send msg', function(data) {
    if (data.user == user) {
            ...
            //nightMode colors
            if(nightMode){
                p.style.backgroundColor = '#1fc587';
            }else{
        ...
        if(nightMode){
            p.style.backgroundColor = '#C51F5D';
        }else{
            p.style.backgroundColor = "#536DAD";
        }
...
btn_send.addEventListener('click', function() {
    if(msg_input.value == ''){
        ...
        error_msg.innerHTML = '';
        msg_input.style.border = '2px solid #006994';
        socket.emit('send msg', msg_input.value);
    }
...
//     let doc = document.querySelector('.imagee');
//     let img1 = document.createElement('img');
//     img1.src = `./photos/${data}`;
